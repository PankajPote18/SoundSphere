...
import AdminAudioForm from './AdminAudioForm';
...
const AdminAudios = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this audio?')) {
      fetch(`${import.meta.env.VITE_API_URL}/api/audios/${id}`, { method: 'DELETE' })
        .then(() => fetchMovies());
    }
  };

  if (showForm) {
    return <AdminAudioForm
      audio={editingMovie}
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h2 className="text-xl md:text-2xl font-bold">Manage Audios</h2>
        <button
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[600px]">
            ...
            <tbody className="divide-y divide-gray-800">
              {audios.map(audio => (
                <tr key={audio.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 font-medium">{audio.title}</td>
                  <td className="px-6 py-4 text-gray-400">{audio.category_id}</td>
                  <td className="px-6 py-4 text-gray-400">{audio.releaseYear}</td>
                  <td className="px-6 py-4 flex items-center space-x-3">
                    <button
                      onClick={() => { setEditingMovie(audio); setShowForm(true); }}
